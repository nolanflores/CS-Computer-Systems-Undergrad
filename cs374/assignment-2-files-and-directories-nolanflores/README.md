...
5. Clone this repository via the following shell command: `git clone <PASTE SSH URL HERE>`. To verify that it worked, run `ls`. You should see a new directory called `assignment-2-files-and-directories-<USERNAME>` (where `<USERNAME>` is your GitHub username). Navigate into that directory via `cd`. It should contain a copy of this repository's contents. You can now proceed to complete `processmovies.c` in a terminal-based text editor, such as `vim`. If `git clone` *didn't* work, then there may be something wrong with your SSH key config. Refer to the Assignment 1 repository's front page / `README.md` file for instructions on how to fix these issues.
