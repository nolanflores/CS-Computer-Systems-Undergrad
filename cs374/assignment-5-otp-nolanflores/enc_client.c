...
int main(int argc, char* argv[]){
	...
	char plaintxt[1026] = {0};
	int pt_len = string_from_file(argv[1], plaintxt);
	if(pt_len == -1)
		return 1;
	char key[1026] = {0};
	...
	send_message(socket_fd, "E@@", 3);
	char server_type[1024] = {0};
	int rec_result = receive_message(socket_fd, server_type);
	if(rec_result == -1)
		return 2;
	if(server_type[0] != 'E'){
		fprintf(stderr, "Server incompatible (expecting encryption server).\n");
		close(socket_fd);
		freeaddrinfo(server_addr_info);
		return 2;
	}
	send_message(socket_fd, plaintxt, pt_len);
	send_message(socket_fd, key, k_len);
	char encrypted_message[1026] = {0};
	rec_result = receive_message(socket_fd, encrypted_message);
	if(rec_result == -1)
		return 2;
	printf("%s\n", encrypted_message);
