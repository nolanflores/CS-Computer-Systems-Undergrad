...
void decrypt(char* encrypted_message, int message_length, char* key, char* message){
	for(int i = 0; i < message_length; i++){
		int e, k;
		if(encrypted_message[i] == ' ')
			e = 26;
		else
			e = ((int)encrypted_message[i])-65;
		if(key[i] == ' ')
			k = 26;
		else
			k = ((int)key[i])-65;
...
int main(int argc, char* argv[]){
	...
	while(1){
		...
		if(child_pid == 0){//child
			char client_type[1026] = {0};
			receive_message(communication_socket_fd, client_type);
			send_message(communication_socket_fd, "D@@");
			if(client_type[0] != 'D'){
				fprintf(stderr, "Client is incompatible (expected decryption client).\n");
			}else{
				char encrypted_message[1026] = {0};
				int m_len = receive_message(communication_socket_fd, encrypted_message);
				char key[1026] = {0};
				receive_message(communication_socket_fd, key);
				char message[1026] = {0};
				decrypt(encrypted_message, m_len, key, message);
				send_message(communication_socket_fd, message);
			}
